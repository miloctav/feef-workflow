...
const { currentEntity, submitCase, approveCase, submitCaseLoading, fetchEntity } = useEntities()
...
// Filtrer pour ne garder que les actions sans auditId (actions niveau entité)
const entityLevelActions = computed(() => {
...
<template>
  ...
  <div
    v-if="isFollowerView && masterEntity"
  ...
  <!-- Liste des actions liées à l'entité (sans audit) -->
  <div
    v-if="displayEntity?.id && entityLevelActions.length > 0"
    class="mb-6"
  >
    <ActionsList
      :key="`entity-actions-${displayEntity.id}`"
      :actions="entityLevelActions"
      :loading="actionsLoading"
      :error="actionsError"
    />
  </div>
  ...
  >
    <UCard>
      <template #header>
        <div class="flex items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-2 flex-1 ml-4">
            ...
            <UBadge
              v-if="latestAudit"
              :color="getAuditStatusColor(latestAudit.status)"
              variant="subtle"
              size="sm"
            >
              <div class="flex items-center gap-1.5">
                <UIcon
                  :name="getAuditStatusIcon(latestAudit.status)"
                  class="w-3 h-3"
                />
            ...
            <!-- Badge Mode -->
            <UBadge
              :color="displayEntity.mode === EntityMode.MASTER ? 'success' : 'info'"
              variant="soft"
              size="sm"
            >
              <div class="flex items-center gap-1.5">
                <UIcon
                  :name="
                    displayEntity.mode === EntityMode.MASTER ? 'i-lucide-star' : 'i-lucide-link'
                  "
                  class="w-3 h-3"
                />
      ...
      <div class="flex gap-6">
        ...
        <div class="flex-1 space-y-6">
          ...
          <div>
            <div class="flex items-center justify-between mb-4">
              ...
              <EditEntityContactModal
                v-if="displayEntity && (user?.role === Role.FEEF || user?.role === Role.ENTITY)"
                :entity-id="displayEntity.id"
                :address="displayEntity.address"
                :address-complement="displayEntity.addressComplement"
                :postal-code="displayEntity.postalCode"
                :city="displayEntity.city"
                :region="displayEntity.region"
                :phone-number="displayEntity.phoneNumber"
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div class="flex items-start gap-2 text-sm">
                  ...
                  <div class="text-gray-700">
                    ...
                    <div
                      v-if="displayEntity.addressComplement"
                      class="text-gray-600"
                    >
                      {{ displayEntity.addressComplement }}
                    </div>
                    <div>
        ...
        <div
          v-if="hasActions"
          class="w-80 flex-shrink-0 pl-6 border-l border-gray-200"
        >
          ...
          <div class="space-y-3">
                  ...
                  <template v-else>
                    ...
                    >
                      <template #description>
                        ...
                        <p class="mt-2">
                          Ce choix ne sera <strong>plus modifiable</strong> après le dépôt du
                          dossier.
                        </p>
                        <p class="mt-2 text-sm">
                          Pour modifier, allez dans l'onglet <strong>Contrats</strong> où vous
                          pouvez changer d'OE ou passer en mode appel d'offre.
                        </p>
                      </template>
                    </UAlert>
                  </template>
            ...
            >
              <div class="flex items-start gap-2">
                <UIcon
                  ...
                  class="w-4 h-4 mt-0.5 flex-shrink-0 text-success"
                />
                <div>
            ...
            <div
              v-if="displayEntity?.accountManager"
              class="text-sm text-gray-600 pt-1"
            >
              <div class="flex items-start gap-2">
                <UIcon
                ...
                <div>
                  ...
                  <div>
                    {{ displayEntity.accountManager.firstname }}
                    {{ displayEntity.accountManager.lastname }}
                  </div>
  ...
  >
      ...
      >
        ...
        <div class="space-y-3">
          ...
          <div class="text-sm text-gray-600">
            {{ getFieldValue(`${groupKey}Role`) || 'Non défini' }}
          </div>
          <div class="flex items-center gap-2">
            <UIcon
              name="i-lucide-phone"
      ...
      <div class="space-y-4">
        <div>
          ...
          <p class="text-sm text-gray-900">
            {{ getFieldValue('labelingScopeRequestedScope') || 'Non défini' }}
          </p>
        </div>

        <!-- Alerte rouge si exclusions -->
        <div
          v-if="getFieldValue('labelingScopeExcludeYesno')"
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4">
        <div
          v-for="field in fieldGroups.find((g) => g.key === 'revenue')?.fields || []"
          :key="field.key"
        >
    ...
    >
      ...
      <div class="space-y-4">
        <div>
          ...
          <p class="text-sm text-gray-900">
            {{ getFieldValue('activitiesDescription') || 'Non défini' }}
          </p>
        </div>
            ...
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    ...
                    getFieldValue('activitiesProductionInhouse') ? 'bg-green-500' : 'bg-gray-300',
                  ]"
                ></div>
              ...
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    'w-3 h-3 rounded-full',
                    getFieldValue('activitiesProductionSubcontracted')
                      ? 'bg-orange-500'
                      : 'bg-gray-300',
                  ]"
                ></div>
            ...
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    'w-3 h-3 rounded-full',
                    getFieldValue('activitiesPackagingInhouse') ? 'bg-green-500' : 'bg-gray-300',
                  ]"
                ></div>
                <span class="text-sm">En propre</span>
              </div>
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    'w-3 h-3 rounded-full',
                    getFieldValue('activitiesPackagingSubcontracted')
                      ? 'bg-orange-500'
                      : 'bg-gray-300',
                  ]"
                ></div>
            ...
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    'w-3 h-3 rounded-full',
                    getFieldValue('activitiesLogisticsInhouse') ? 'bg-green-500' : 'bg-gray-300',
                  ]"
                ></div>
                <span class="text-sm">En propre</span>
              </div>
              <div class="flex items-center gap-2">
                <div
                  :class="[
                    'w-3 h-3 rounded-full',
                    getFieldValue('activitiesLogisticsSubcontracted')
                      ? 'bg-orange-500'
                      : 'bg-gray-300',
                  ]"
                ></div>
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <div>
            <h5 class="text-sm font-medium text-gray-700 mb-3">
              Répartition du CA {{ groupKey === 'subcontracting' ? 'sous-traitance' : 'négoce' }}
            </h5>
            <div class="space-y-2">
              ...
              <div class="w-full bg-gray-200 rounded-full h-2">
                <div
                  :class="groupKey === 'subcontracting' ? 'bg-purple-600' : 'bg-orange-600'"
                  class="h-2 rounded-full transition-all duration-300"
                  :style="`width: ${getFieldValue(`${groupKey}TotalRatio`) || 0}%`"
                ></div>
              </div>
            </div>
          </div>
            ...
            <div class="flex flex-wrap gap-4 mb-3">
              <div class="flex items-center gap-2 text-xs">
                ...
                <span class="text-gray-600"
                  >France: {{ getFieldValue(`${groupKey}FranceRatio`) || 0 }}%</span
                >
              </div>
              <div class="flex items-center gap-2 text-xs">
                <div class="w-3 h-3 bg-green-500 rounded"></div>
                <span class="text-gray-600"
                  >Europe: {{ getFieldValue(`${groupKey}EuropeRatio`) || 0 }}%</span
                >
              </div>
              <div class="flex items-center gap-2 text-xs">
                <div class="w-3 h-3 bg-orange-500 rounded"></div>
                <span class="text-gray-600"
                  >Hors Europe: {{ getFieldValue(`${groupKey}NonEuropeRatio`) || 0 }}%</span
                >
              </div>
            </div>
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
              <div class="h-full flex">
                <div
                  ...
                  :style="`width: ${getFieldValue(`${groupKey}FranceRatio`) || 0}%`"
                ></div>
                <div
                  class="bg-green-500 h-full transition-all duration-300"
                  :style="`width: ${getFieldValue(`${groupKey}EuropeRatio`) || 0}%`"
                ></div>
                <div
                  class="bg-orange-500 h-full transition-all duration-300"
                  :style="`width: ${getFieldValue(`${groupKey}NonEuropeRatio`) || 0}%`"
                ></div>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="flex items-center gap-2">
                <span class="text-2xl font-bold text-green-600">{{
                  getFieldValue('productsRangeCount') || 0
                }}</span>
            ...
            <div>
              ...
              <p class="text-sm text-gray-900">
                {{ getFieldValue('productsBrands') || 'Non défini' }}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-4 mb-3">
              <div class="flex items-center gap-2 text-xs">
                <div class="w-3 h-3 bg-green-500 rounded"></div>
                <span class="text-gray-600"
                  >Alimentaires: {{ getFieldValue('productsFoodRatio') || 0 }}%</span
                >
              </div>
              <div class="flex items-center gap-2 text-xs">
                <div class="w-3 h-3 bg-blue-500 rounded"></div>
                <span class="text-gray-600"
                  >Non-alimentaires: {{ getFieldValue('productsNonFoodRatio') || 0 }}%</span
                >
              </div>
            </div>
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
              <div class="h-full flex">
                <div
                  class="bg-green-500 h-full transition-all duration-300"
                  :style="`width: ${getFieldValue('productsFoodRatio') || 0}%`"
                ></div>
                <div
                  class="bg-blue-500 h-full transition-all duration-300"
                  :style="`width: ${getFieldValue('productsNonFoodRatio') || 0}%`"
                ></div>
              </div>
            </div>
          </div>
    ...
    <UCard
      ...
      @click="
        user?.role === Role.FEEF || user?.role === Role.ENTITY
          ? openFieldGroupEditor('employees')
          : undefined
      "
    >
      <template #header>
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-xl">
              {{ getFieldValue('employeesTotalScopeCount') || 0 }} Salariés
            </h2>
          </div>
          <UIcon
            v-if="user?.role === Role.FEEF || user?.role === Role.ENTITY"
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        <div class="flex items-center gap-6">
          <div class="relative w-32 h-32 flex-shrink-0">
            ...
            >
              ...
              <circle
                ...
                stroke-width="3"
                :stroke-dasharray="`${
                  ((getFieldValue('employeesAdminCount') || 0) /
                    (getFieldValue('employeesTotalScopeCount') || 1)) *
                  100
                } ${
                  100 -
                  ((getFieldValue('employeesAdminCount') || 0) /
              ...
              <circle
                ...
                stroke-width="3"
                :stroke-dasharray="`${
                  ((getFieldValue('employeesProductionCount') || 0) /
                    (getFieldValue('employeesTotalScopeCount') || 1)) *
                  100
                } ${
                  100 -
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="text-center">
                <div class="text-xl font-bold text-gray-900">
                  {{ getFieldValue('employeesTotalScopeCount') || 0 }}
                </div>
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="text-gray-600 text-sm mt-3 pt-2 border-t">
              <span class="font-medium">Personnel siège:</span>
              {{ getFieldValue('employeesHeadcountHq') || 0 }}
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2">
          <div class="flex items-center gap-3 p-2 bg-gray-50 rounded-lg w-fit">
            ...
            <UBadge
              :variant="getFieldValue('employeesCsePresent') ? 'solid' : 'outline'"
              :color="getFieldValue('employeesCsePresent') ? 'success' : 'neutral'"
              size="sm"
            >
          ...
          <div class="flex items-center gap-3 p-2 bg-gray-50 rounded-lg w-fit">
            ...
            <span class="font-semibold text-gray-900 text-sm"
              >{{ getFieldValue('employeesRseResources') || 0 }} ETP/an</span
            >
          </div>
        </div>
      </div>
    ...
    <UCard
      ...
      @click="
        user?.role === Role.FEEF || user?.role === Role.ENTITY
          ? openFieldGroupEditor('bio_activities')
          : undefined
      "
    >
      <template #header>
      ...
      <div class="space-y-6">
        ...
        <div class="flex items-center gap-3 p-4 bg-green-50 rounded-lg border border-green-200">
          ...
          <div class="flex-1">
            ...
            <UBadge
              :color="getFieldValue('bioLabelPresent') ? 'success' : 'neutral'"
              variant="solid"
            >
          ...
          <div class="space-y-3">
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-2"
                >Labellisation principale</label
              >
