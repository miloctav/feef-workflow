...
<template>
  <NuxtLink v-if="to" :to="to" class="block w-full">
    <UPageCard
      class="w-full py-0 cursor-pointer transition-shadow hover:shadow-md hover:ring-1 hover:ring-primary-200"
      spotlight
    >
      <template #default>
        <div class="flex flex-row gap-2 items-start w-full">
          ...
          <div class="flex flex-col flex-1 min-w-0 w-full">
            <span class="text-sm text-muted truncate">{{ shortText }}</span>
            <div
              v-if="auditTypes && (auditTypes.initial > 0 || auditTypes.renewal > 0 || auditTypes.monitoring > 0)"
              class="flex flex-row gap-1 mt-0.5 flex-wrap"
            >
              <UBadge v-if="auditTypes.initial > 0" class="bg-blue-100 text-blue-700 border-none text-xs px-1.5 py-0.5">{{ auditTypes.initial }} Initial</UBadge>
              <UBadge v-if="auditTypes.renewal > 0" class="bg-green-100 text-green-700 border-none text-xs px-1.5 py-0.5">{{ auditTypes.renewal }} Renouvellement</UBadge>
              <UBadge v-if="auditTypes.monitoring > 0" class="bg-purple-100 text-purple-700 border-none text-xs px-1.5 py-0.5">{{ auditTypes.monitoring }} Suivi</UBadge>
            </div>
            <div
              v-if="(alertesRouges && alertesRouges > 0) || (alertesOranges && alertesOranges > 0)"
              class="flex flex-row gap-1 mt-0.5 flex-wrap"
            >
              <UBadge v-if="alertesRouges && alertesRouges > 0" class="bg-red-100 text-red-700 border-none text-xs px-1.5 py-0.5">{{ alertesRouges }} alertes</UBadge>
              <UBadge v-if="alertesOranges && alertesOranges > 0" class="bg-orange-100 text-orange-700 border-none text-xs px-1.5 py-0.5">{{ alertesOranges }} alertes</UBadge>
            </div>
          </div>
        </div>
      </template>
