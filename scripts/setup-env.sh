...
sed -i "s|MINIO_ENDPOINT=.*|MINIO_ENDPOINT=$MINIO_ENDPOINT|g" "$ENV_FILE"
sed -i "s|MINIO_BUCKET=.*|MINIO_BUCKET=$MINIO_BUCKET|g" "$ENV_FILE"

sed -i "s|NUXT_SESSION_PASSWORD=.*|NUXT_SESSION_PASSWORD=$NUXT_SESSION_PASSWORD|g" "$ENV_FILE"
sed -i "s|JWT_SECRET=.*|JWT_SECRET=$JWT_SECRET|g" "$ENV_FILE"

if [ "$ENVIRONMENT" = "production" ]; then
    sed -i "s|DOMAIN=.*|DOMAIN=$DOMAIN|g" "$ENV_FILE"
    sed -i "s|EMAIL=.*|EMAIL=$EMAIL|g" "$ENV_FILE"
fi
...
if [ -n "$RESEND_API_KEY" ]; then
    ...
    sed -i "s|RESEND_FROM_EMAIL=.*|# RESEND_FROM_EMAIL=|g" "$ENV_FILE"
fi

if [ "$ENVIRONMENT" = "development" ]; then
    sed -i "s|# DEV_MODE=.*|DEV_MODE=$DEV_MODE|g" "$ENV_FILE"
    sed -i "s|DEV_MODE=.*|DEV_MODE=$DEV_MODE|g" "$ENV_FILE"
fi
